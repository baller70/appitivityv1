...
import { ServerRuntimeClient, applySdkMetadata, setAsyncContextStrategy, spanToJSON, SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, hasSpansEnabled, spanToTraceContext, getRootSpan, getDynamicSamplingContextFromSpan, getCurrentScope, getCapturedScopesOnSpan, dynamicSamplingContextToSentryBaggageHeader, generateSentryTraceHeader, handleCallbackErrors, baggageHeaderToDynamicSamplingContext, parseSampleRate, sampleSpan, logger, SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, LRUMap, getClient, parseBaggageHeader, SENTRY_BAGGAGE_KEY_PREFIX, getDynamicSamplingContextFromScope, SDK_VERSION, SEMANTIC_ATTRIBUTE_SENTRY_OP, getDefaultIsolationScope, getDefaultCurrentScope, timedEventsToMeasurements, captureEvent, addChildSpanToSpan, setCapturedScopesOnSpan, logSpanStart, logSpanEnd, stringMatchesSomePattern, getIsolationScope, propagationContextFromHeaders, convertSpanLinksForEnvelope, getStatusMessage, spanTimeInputToSeconds, SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN, SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME, addNonEnumerableProperty, SPAN_STATUS_OK, SPAN_STATUS_ERROR, parseUrl, getSanitizedUrlString, stripUrlQueryAndFragment, getSpanStatusFromHttpCode, defineIntegration, addFetchInstrumentationHandler, isSentryRequestUrl, instrumentFetchRequest, addBreadcrumb, getBreadcrumbLogLevelFromHttpStatusCode, createTransport, suppressTracing as suppressTracing$2, SENTRY_BUFFER_FULL_ERROR, GLOBAL_OBJ, createStackParser, nodeStackLineParser, dedupeIntegration, inboundFiltersIntegration, functionToStringIntegration, linkedErrorsIntegration, consoleIntegration, requestDataIntegration, getIntegrationsToSetup, stackParserFromStackParserOptions, _INTERNAL_captureLog, fmt } from '@sentry/core';
...
import { createContextKey, baggageEntryMetadataFromString, propagation, diag, DiagLogLevel, trace as trace$1, isSpanContextValid, TraceFlags, SpanStatusCode, isValidTraceId, context, SpanKind, INVALID_TRACEID, ROOT_CONTEXT } from '@opentelemetry/api';
...
var W3CTraceContextPropagator = /** @class */ (function () {
    ...
    W3CTraceContextPropagator.prototype.inject = function (context, carrier, setter) {
        var spanContext = trace$1.getSpanContext(context);
        if (!spanContext ||
    ...
    W3CTraceContextPropagator.prototype.extract = function (context, carrier, getter) {
        ...
        return trace$1.setSpanContext(context, spanContext);
    };
...
var ParentBasedSampler = /** @class */ (function () {
    ...
    ParentBasedSampler.prototype.shouldSample = function (context, traceId, spanName, spanKind, attributes, links) {
        var parentContext = trace$1.getSpanContext(context);
...
var BasicTracerProvider = /** @class */ (function () {
    ...
    BasicTracerProvider.prototype.register = function (config) {
        if (config === void 0) { config = {}; }
        trace$1.setGlobalTracerProvider(this);
...
class SentryPropagator extends W3CBaggagePropagator {
   ...
   */
   inject(context, carrier, setter) {
    ...
    const activeSpan = trace$1.getSpan(context);
...
function shouldPropagateTraceForUrl(
  ...
  if (cachedDecision !== undefined) {
    DEBUG_BUILD$1 && !cachedDecision && logger.log(NOT_PROPAGATED_MESSAGE, url);
  ...
  DEBUG_BUILD$1 && !decision && logger.log(NOT_PROPAGATED_MESSAGE, url);
  return decision;
}
...
function getInjectionData(context)
 ...
 {
  const span = trace$1.getSpan(context);
...
function getContextWithRemoteActiveSpan(
  ...
  return trace$1.setSpanContext(ctx, spanContext);
}
...
function startSpan(options, callback) {
  ...
  return wrapper(() => {
    ...
    const shouldSkipSpan = options.onlyIfParent && !trace$1.getSpan(activeCtx);
...
function withActiveSpan(span, callback) {
  const newContextWithActiveSpan = span ? trace$1.setSpan(context.active(), span) : trace$1.deleteSpan(context.active());
  return context.with(newContextWithActiveSpan, () => callback(getCurrentScope()));
}

function getTracer() {
  const client = getClient();
  return client?.tracer || trace$1.getTracer('@sentry/opentelemetry', SDK_VERSION);
}
...
function getContext(scope, forceTransaction) {
  ...
  const parentSpan = trace$1.getSpan(ctx);
  ...
  const ctxWithoutSpan = trace$1.deleteSpan(ctx);
  ...
  const ctxWithSpanContext = trace$1.setSpanContext(ctxWithoutSpan, spanOptions);
...
function onSpanStart(span, parentContext) {
  ...
  const parentSpan = trace$1.getSpan(parentContext);
  ...
  /** @inheritDoc */
   shouldSample(
    ...
    if (
      ...
      DEBUG_BUILD$1 && logger.log('[Tracing] Discarding root span because its trace was not chosen to be sampled.');
...
function getBaseTraceState(context, spanAttributes) {
  const parentSpan = trace$1.getSpan(context);
  const parentContext = parentSpan?.spanContext();
...
function setupOtel(client) {
  ...
  trace$1.setGlobalTracerProvider(provider);
...
export { VercelEdgeClient, getDefaultIntegrations, init, exports as logger, winterCGFetchIntegration };
